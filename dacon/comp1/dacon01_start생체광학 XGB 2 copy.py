...
# 2. model
model = MultiOutputRegressor(XGBRegressor(gpu_id=0, tree_method='gpu_hist'))
...
# AttributeError: 'GridSearchCV' object has no attribute 'estimators_'
    ...
    selects.append(select_x_test)
    # selects = select_x_test
    
    y_predict = MOR.predict(select_x_test)
...
best_select = selects[best_idx]
y_predict = best_model.predict(best_select)
...
y_pred.to_csv('./dacon/comp1/y_pred2.csv', index = False)
