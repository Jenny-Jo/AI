...
plt.subplot(2, 1, 1)                #2행 1열의 첫번째 그림 그리겠다. 인덱스 1부터 시작한다

plt.plot(hist.history['loss'], marker='.', c = 'red', label='loss') # 히스토리에 저장된 애들을 끄집어온다.
                                                                #  loss'값을 y로 넣겠다./ 하나만 쓰면 y 값으로 들어감
plt.plot(hist.history['val_loss'], marker='.', c='blue', label='val_lass')# 시간에 따른 loss, acc여서 x 값으로는 자연스럽게 epoch가 들어감
plt.plot(hist.history['acc'])
plt.plot(hist.history['val_acc'])
plt.grid()                          #모눈종이 처럼 가로세로 줄 그어서 만듬
plt.title('loss')
plt.ylabel('loss')
plt.xlabel('epoch')
# plt.legend(['loss', 'val_loss'])    #기호 설명표/ plot 첫번째, 두번째 순
plt.legend(loc='upper right')         # legend 자리 명시해줌


plt.subplot(2, 1, 2)                 #2행 1열의 두번째 그림 그리겠다.
