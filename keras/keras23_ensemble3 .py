...
import numpy as np
x1 = np.array([range(1,101), range(311,411),range(411,511)]).T #1~100 (미만으로 보자!) 한두개 틀려도 돌아감. 
x2 = np.transpose([range(101,201),range(711,811),range(511,611)]) #0~99,

y1 = np.array([range(101,201), range(411,511),range(100)]).reshape(100,3)
...
########## model range ##############
model = Model(inputs=[input1,input2], outputs=output1_3) #함수형 모델 범위 하단에 명시, 함수형모델 이름을 model 소문자로 씀/제일 처음, 제일 끝
